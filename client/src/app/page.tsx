...
import { useVisitCount, useTrackVisit } from "@/hooks/use-visits";
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none">
      {particles.map((i) => (
        <motion.div
          key={i}
          className="absolute w-1 h-1 bg-gradient-to-r from-purple-500/20 to-fuchsia-500/20 rounded-full"
          style={{
            left: `${(i * 37 + 13) % 100}%`,
            top: `${(i * 53 + 7) % 100}%`,
          }}
...
function GradientOrbs() {
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none">
      <motion.div
        className="absolute -top-40 -right-40 w-96 h-96 bg-gradient-to-br from-purple-500/30 to-fuchsia-500/30 rounded-full blur-3xl"
        animate={{
          scale: [1, 1.2, 1],
          x: [0, -50, 0],
          y: [0, 30, 0],
      ...
      />
      <motion.div
        className="absolute -bottom-40 -left-40 w-[500px] h-[500px] bg-gradient-to-tr from-blue-500/20 to-cyan-500/20 rounded-full blur-3xl"
        animate={{
          scale: [1.2, 1, 1.2],
          x: [0, 50, 0],
          y: [0, -50, 0],
      ...
      />
      <motion.div
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-gradient-to-r from-pink-500/10 to-rose-500/10 rounded-full blur-3xl"
        animate={{
          scale: [1, 1.3, 1],
          opacity: [0.3, 0.5, 0.3],
        }}
          ...
          <div className="flex items-start gap-4">
            <Link href={`/apps/${app.id}`} className="shrink-0">
              ...
              >
                ...
                <img
                  src={app.iconUrl}
                  alt={app.name}
                  className="w-16 h-16 rounded-2xl object-cover relative z-10 shadow-lg"
                />
              </motion.div>
            </Link>
            ...
            <div className="flex-1 min-w-0">
              <Link href={`/apps/${app.id}`}>
                <motion.h3
                  className="text-lg font-semibold text-white group-hover:text-purple-400 transition-colors truncate"
                  whileHover={{ x: 5 }}
                >
                  {app.name}
              ...
              <p className="text-sm text-slate-400 line-clamp-2 mt-1 group-hover:text-slate-300 transition-colors">
                {app.description}
              </p>
              ...
              <div className="flex items-center gap-3 mt-2.5">
                <Badge
                  variant="secondary"
                  className="text-xs bg-slate-800/50 border-slate-700 group-hover:bg-purple-500/20 group-hover:border-purple-500/30 transition-colors"
                >
                  {app.categoryName}
                </Badge>
                <span className="text-xs text-slate-500 flex items-center gap-1 group-hover:text-green-400 transition-colors">
            ...
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button
                ...
                className="shrink-0 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 shadow-lg shadow-green-500/25 hover:shadow-green-500/40 px-5 h-10"
              >
      ...
      <div className="container mx-auto px-4 text-center">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-purple-500/10 border border-purple-500/20 mb-6">
            ...
            <span className="text-sm text-purple-300">El mejor catálogo de aplicaciones</span>
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center gap-2 px-5 py-3 rounded-xl bg-slate-900/50 border border-slate-800">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Aplicaciones</p>
            </div>
          </div>
          <div className="flex items-center gap-2 px-5 py-3 rounded-xl bg-slate-900/50 border border-slate-800">
            ...
            <div>
              <p className="text-2xl font-bold text-white">{totalDownloads.toLocaleString("es-ES")}</p>
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
          <div>
            ...
            <p className="text-slate-400 text-sm">
              Tu destino premium para descubrir y descargar las mejores aplicaciones y juegos.
            </p>
          </div>
          <div>
            ...
            <ul className="space-y-2 text-sm text-slate-400">
              ...
              <li><Link href="/?category=juegos" className="hover:text-purple-400 transition-colors">Juegos</Link></li>
              <li><Link href="/?category=aplicaciones" className="hover:text-purple-400 transition-colors">Aplicaciones</Link></li>
              <li><Link href="/?category=popular" className="hover:text-purple-400 transition-colors">Populares</Link></li>
            </ul>
          </div>
          <div>
        ...
        <div className="border-t border-slate-800/50 pt-8 text-center">
          <p className="text-sm text-slate-500">
            © {new Date().getFullYear()} App VIP. Todos los derechos reservados.
          </p>
        </div>
      </div>
    </footer>
...
export default function HomePage() {
  ...
  const { data: visitData } = useVisitCount();
  ...
  const handleCategoryChange = useCallback((catId: CategoryFilter) => {
    ...
    if (catId !== "all" && catId !== "popular" && catId !== "most-downloaded") {
      setSearchTerm("");
    }
  }, []);
      ...
      <header className="sticky top-0 z-50 bg-slate-950/80 backdrop-blur-xl border-b border-slate-800/50">
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 group">
            <motion.div
              className="p-2 bg-gradient-to-br from-purple-500 to-fuchsia-600 rounded-xl shadow-lg shadow-purple-500/20"
          ...
          <div className="flex items-center gap-3">
            {visitData && (
              <motion.div
                className="hidden md:flex items-center gap-2 text-sm text-slate-400 px-3 py-1.5 rounded-lg bg-slate-900/50 border border-slate-800"
                initial={{ opacity: 0, x: -10 }}
                animate={{ opacity: 1, x: 0 }}
              >
                <Eye className="w-4 h-4 text-purple-400" />
                <span>{visitData.visits.toLocaleString("es-ES")} visitas</span>
              </motion.div>
            )}
            ...
            {isAuthenticated ? (
                ...
                {isAdmin && (
                  <Link href="/admin">
                    ...
                    >
                      <Button
                        size="sm"
                        className="bg-gradient-to-r from-amber-500 to-yellow-600 hover:from-amber-600 hover:to-yellow-700 text-black font-semibold shadow-lg shadow-amber-500/20"
                      >
              ...
              <div className="flex items-center gap-2">
                ...
                <Link href="/register">
                  <Button size="sm" className="bg-gradient-to-r from-purple-500 to-fuchsia-600 shadow-lg shadow-purple-500/20 hover:shadow-purple-500/40">
                    Registrarse
                  </Button>
                </Link>
              </div>
          ...
          {isLoading ? (
            ...
            <>
              {searchTerm && (
                ...
                >
                  {filteredApps.length} resultado{filteredApps.length !== 1 ? "s" : ""} para &ldquo;{searchTerm}&rdquo;
                </motion.p>
              )}
            ...
            >
              ...
              <p className="text-slate-400 text-lg mb-2">
                {searchTerm ? `No se encontraron resultados para "${searchTerm}"` : "No se encontraron aplicaciones"}
              </p>
              <p className="text-slate-600 text-sm">
                {searchTerm ? "Intenta con otro término de búsqueda" : "Vuelve pronto para nuevas apps"}
              </p>
            </motion.div>
          )}
        </div>
